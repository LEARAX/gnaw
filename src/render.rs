...
pub fn draw_duration<B>(term: &mut Frame<B>, area: Rect, current_song: Option<&gnaw::Song>)
where
    B: tui::backend::Backend,
{
    if let Some(song) = current_song {
        Gauge::default()
            ...
            .label("")
            .ratio(song.elapsed / song.duration)
            .render(term, area);
    } else {
        Block::default()
            .borders(Borders::ALL)
    ...
    B: tui::backend::Backend,
{
    let status_text = if let Some(song) = current_song {
        vec![
            Text::styled(&song.title, Style::default().fg(Color::LightBlue)),
            Text::raw(" | "),
            Text::styled(&song.album, Style::default().fg(Color::LightRed)),
            Text::raw(" | "),
            Text::styled(&song.artist, Style::default().fg(Color::LightYellow)),
        ]
    } else {
        vec![Text::raw("")]
    };
